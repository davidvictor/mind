---
description: Claude-owned onboarding workflow over backend-only mind onboard commands
argument-hint: <path-to-onboarding-json>
---

Use this workflow when the owner wants to onboard or resume onboarding.

Claude is the interaction owner. `mind onboard` is the backend state machine.
...
- `.venv/bin/python -m mind onboard --from-json <path>` is compatibility-only and skips the question-by-question Claude workflow.
