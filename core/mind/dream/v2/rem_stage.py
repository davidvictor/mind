...
from mind.dream.quality import QUALITY_ADAPTER, evaluate_and_persist_quality, lane_state_for_summary_id, supports_full_dream_mutation
from mind.dream.substrate_queries import active_atoms, atom_path, touched_active_atoms
    ...
    *,
    ...
    with dream_run("rem", dry_run=dry_run, context=context) as (runtime, run_id):
        ...
        with maybe_locked("rem", dry_run=dry_run, acquire_lock=acquire_lock):
            ...
            reflection_package = _build_reflection_package(
                month=month,
                hotset=hotset,
                clusters=clusters,
                blocked=prune_brake,
            )
            reflection_target = v.wiki / "me" / "reflections" / f"{month}.md"
            review_nudges = _reflection_review_nudges(month=month, reflection_package=reflection_package)
            apply_plan = ApplyPlan(
                run_id=artifact_run_id,
                stage="rem",
                mode="shadow" if dry_run else "write",
                actions=[
                    ApplyPlanAction(
                        action_id=f"write-reflection-{month}",
                        action_type="write_markdown",
                        target_path=v.logical_path(reflection_target),
                        safe_to_apply=not dry_run,
                        rationale="monthly REM reflection page",
                    ),
                    *[
                        ApplyPlanAction(
                            action_id=nudge["id"],
                ...
                notes=[
                    f"hotset atoms: {len(hotset)}",
                    f"clusters: {len(clusters)}",
                ],
            )

            mutations.append(
                f"{'would write' if dry_run else 'wrote'} Dream v2 REM dossier "
                f"{write_stage_json(layout, 'dossier.json', dossier, dry_run=dry_run)}"
            )
            mutations.append(
                f"{'would write' if dry_run else 'wrote'} Dream v2 REM reflection package "
                f"{write_stage_json(layout, 'reflection-package.json', reflection_package, dry_run=dry_run)}"
            )
            mutations.append(
                f"{'would write' if dry_run else 'wrote'} Dream v2 REM apply plan "
                f"{write_stage_json(layout, 'apply-plan.json', apply_plan, dry_run=dry_run)}"
            )

            apply_entries: list[ApplyManifestEntry] = []
            if dry_run:
                if effective_write_rem_page:
                    mutations.append(f"would write REM reflection page {reflection_target.relative_to(v.wiki)}")
                for nudge in review_nudges:
                    mutations.append(f"would write REM review nudge {nudge['target_path']}")
                ...
                write_note_page(
                    reflection_target,
                    page_type="note",
                    title=f"REM Reflection {month}",
                    body=_render_reflection_page(reflection_package),
                    domains=["identity", "dream"],
                    extra_frontmatter={
                        "origin": "dream.rem.v2",
                        "kind": "reflection",
                        "relates_to": [f"[[{item['atom_id']}]]" for item in reflection_package["focus_atoms"][:8]],
                    },
                    force=True,
                    context=context,
                )
                mutations.append(f"wrote REM reflection page {reflection_target.relative_to(v.wiki)}")
                apply_entries.append(
                    ApplyManifestEntry(
                        action_id=f"write-reflection-{month}",
                        status="written",
                        target_path=v.logical_path(reflection_target),
                        notes=["monthly REM reflection page"],
                    )
                )
                for nudge in review_nudges:
                    target_path = v.resolve_logical_path(nudge["target_path"])
                    write_note_page(
                        target_path,
                        page_type="note",
                        title=nudge["title"],
                    ...
                    mutations.append(f"wrote REM review nudge {nudge['target_path']}")
                    apply_entries.append(
                        ApplyManifestEntry(
                            action_id=nudge["id"],
                            status="written",
            ...
            apply_manifest = ApplyManifest(
                ...
                write_count=sum(1 for entry in apply_entries if entry.status == "written"),
                warning_count=sum(1 for entry in apply_entries if entry.status != "written"),
                notes=list(apply_plan.notes),
            )
            mutations.append(
                f"{'would write' if dry_run else 'wrote'} Dream v2 REM apply manifest "
                ...
                DreamRunManifest(
                    ...
                    stages=[
                        StageRunSummary(
                            stage="rem",
                            status="completed",
                            candidate_count=len(hotset),
                            decision_artifact_count=2,
        ...
        "clusters": [
            {
                ...
                "score": cluster.score,
                "relation_edges": cluster.relation_edges,
                "life_mentions": cluster.life_mentions,
            }
            for cluster in clusters
        ],
        "prune_candidates": [
            {"action": action, "atom_id": score.atom_id, "prune_score": score.prune_score}
            for action, score in prune_candidates
        ],
        "merge_candidates": [
            {"winner_atom_id": winner.atom_id, "loser_atom_id": loser.atom_id}
            for winner, loser in merge_candidates
        ],
    }


def _build_reflection_package(
    *,
    month: str,
    hotset: list[RemAtomScore],
    clusters: list[RemCluster],
    ...
    values_pressure = [
        {
            "atom_id": item.atom_id,
            "why": f"life mentions={item.life_mentions} new evidence={item.new_evidence_count}",
        }
        for item in hotset
        if item.life_mentions > 0
    ][:5]
    positioning_pressure = [
        {
            "atom_id": item.atom_id,
            "why": f"relation degree={len(item.relation_ids)} hot score={item.hot_score}",
        }
        for item in sorted(hotset, key=lambda item: (len(item.relation_ids), item.hot_score), reverse=True)
    ][:5]
    inquiry_pressure = [
        {
            "atom_id": item.atom_id,
            "why": f"new evidence={item.new_evidence_count} contradictions={item.contradiction_count}",
        }
        for item in hotset
        if item.atom_type == "inquiry"
    ][:5]
    stance_review = [
        {
            "atom_id": item.atom_id,
            "why": f"contradictions={item.contradiction_count} hot score={item.hot_score}",
        }
        for item in hotset
        if item.atom_type == "stance" or item.contradiction_count > 0
    ][:5]
    return {
        "month": month,
        "blocked": blocked,
        "focus_atoms": [
            ...
            for item in hotset[:8]
        ],
        "cluster_count": len(clusters),
        "values_pressure": values_pressure,
        "positioning_pressure": positioning_pressure,
        "inquiry_pressure": inquiry_pressure,
        "stance_review": stance_review,
        "summary": (
            f"{len(hotset)} hot atoms surfaced in REM, across {len(clusters)} clusters"
            + (" while prune actions stayed report-only." if blocked else ".")
        ),
    }


def _render_reflection_page(reflection_package: dict[str, Any]) -> str:
    lines = [
        f"# REM Reflection {reflection_package['month']}",
        "",
        "## Summary",
        "",
        str(reflection_package["summary"]),
        "",
        "## Focus atoms",
        "",
    ]
    for item in reflection_package["focus_atoms"]:
        lines.append(f"- [[{item['atom_id']}]] ({item['atom_type']}) — hot_score={item['hot_score']}")
    lines.extend(["", "## Values pressure", ""])
    for item in reflection_package["values_pressure"]:
        lines.append(f"- [[{item['atom_id']}]] — {item['why']}")
    lines.extend(["", "## Positioning pressure", ""])
    for item in reflection_package["positioning_pressure"]:
        lines.append(f"- [[{item['atom_id']}]] — {item['why']}")
    lines.extend(["", "## Inquiry pressure", ""])
    for item in reflection_package["inquiry_pressure"]:
        lines.append(f"- [[{item['atom_id']}]] — {item['why']}")
    lines.extend(["", "## Stance review", ""])
    for item in reflection_package["stance_review"]:
        lines.append(f"- [[{item['atom_id']}]] — {item['why']}")
    return "\n".join(lines).rstrip() + "\n"


def _reflection_review_nudges(*, month: str, reflection_package: dict[str, Any]) -> list[dict[str, str]]:
    nudges: list[dict[str, str]] = []
    for category in ("values_pressure", "positioning_pressure", "inquiry_pressure", "stance_review"):
        for item in reflection_package.get(category, [])[:2]:
            atom_id = str(item["atom_id"])
            nudges.append(
                {
                    "id": f"{month}-rem-review-{atom_id}",
                    "title": f"Review {atom_id} after REM",
                    "body": item["why"],
                    "target_path": f"memory/inbox/nudges/{month}-rem-review-{atom_id}.md",
                }
            )
